...
**Улучшение точки отсчёта простоя.**

В v1.0 время простоя считалось от последнего события в логе gateway. Но перезапуск сервиса — это технический момент, не реальное молчание агента.

В v1.1 первым шагом запрашивается `sessions_history` — история сообщений из Telegram-чата. Это даёт точную точку отсчёта: последний ответ агента пользователю. Время простоя теперь = реальное молчание, а не системное событие.

Алгоритм v1.1:
1. `sessions_history` → найти последнее сообщение агента в чате
2. Лог gateway → найти причину сбоя
3. Вычислить простой: от последнего ответа до текущего момента
